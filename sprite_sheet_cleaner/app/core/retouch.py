...
def apply_retouch_stroke(
    ...
    *,
    ...
    for point_x, point_y in points:
        mask = _brush_mask((width, height), (point_x, point_y), radius, opacity)
        if not np.any(mask):
            continue
        if mode == "erase":
...
def _brush_mask(
    ...
    x, y = center
    draw.ellipse((x - radius, y - radius, x + radius, y + radius), fill=round(opacity * 255))
    if radius >= 3:
        mask = mask.filter(ImageFilter.GaussianBlur(max(0.5, radius * 0.18)))
